...
var Game = new function () {
  ...
  function advanceClockQueue() {
    game.clockQueue = game.clockQueue || [];

    const recheckTimeMs = 50;

    if (game.clockQueue.length > 0) {
      window.setTimeout(
        function () {
          game.clockQueue[0][0](),
          game.clockQueue.unshift();
          advanceClockQueue();
        }, 
        game.clockQueue[0][1]
      );
  ...
  function getWinningTeam() {
    ...
    if ( (!teamOneAlive) || (!teamTwoAlive) ) {
      ...
      if (!teamOneAlive && !teamTwoAlive) {
        ...
        game.teamOneWinRecord[0]++;
        return 1;
      } else if (teamTwoAlive) {
        game.teamOneWinRecord[1]++;
        return 2;
      }
    }
  ...
  function gameLoop() {
    ...
    // Phases are in their own file, Phases.js
    var p = Phases;
    ...
    while (game.winningTeam == undefined) {
      // stage 1: flop
      thenTriggerByClock(p.revealPhaseStage1, game.clock.revealPhase);
      thenTriggerByClock(p.betPhase, game.clock.betPhase);
      thenTriggerByClock(p.matchPhase, game.clock.matchPhase);
      thenTriggerByClock(p.collectPhase, game.clock.collectPhase);

      // stage 2: turn
      thenTriggerByClock(p.revealPhaseStage2, game.clock.revealPhase);
      thenTriggerByClock(p.betPhase, game.clock.betPhase);
      thenTriggerByClock(p.matchPhase, game.clock.matchPhase);
      thenTriggerByClock(p.collectPhase, game.clock.collectPhase);

      // stage 4: river
      thenTriggerByClock(p.revealPhaseStage3, game.clock.revealPhase);
      thenTriggerByClock(p.betPhase, game.clock.betPhase);
      thenTriggerByClock(p.matchPhase, game.clock.matchPhase);
      thenTriggerByClock(p.collectPhase, game.clock.collectPhase);

      // stage 5: showdown
      thenTriggerByClock(p.revealPhaseStage4, game.clock.revealPhase);
      thenTriggerByClock(p.winnerPhase, game.clock.winnerPhase);

      thenTriggerByClock(p.choosePhase, game.clock.choosePhase);
      thenTriggerByClock(p.castPhase, game.clock.castPhase);

      game.winningTeam = getWinningTeam();
    }

    thenTriggerByClock(p.victoryPhase, game.clock.victoryPhase);
    thenTriggerByClock(p.replayPhase, game.clock.replayPhase);
  }
  ...
  function triggerByClock(callback, time) {
    ...
    const SKIP_STACK_LIMIT = 20;
    // this is how many actions we're willing to perform
    // in the same execution frame, without placing the next
    // thing on the stack, if there are many things happening
    // with an intended delay of 0.
  ...
  function thenTriggerByClock(callback, time) {
    game.clockQueue = game.clockQueue || [];
    game.clockQueue.push([callback, time]);
  }
